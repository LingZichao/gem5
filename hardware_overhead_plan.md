...
- `util/uacc_hw/trace_schema.md`：packet/window/candidate trace 格式和单位；
- `util/uacc_hw/export_uacc_trace.py`：将 gem5 统计转换为可回放输入，或在需要时增加轻量 gem5 trace hook；
- `util/uacc_hw/fixed_point_model.py`：逐位等价黄金模型和格式 sweep；
- `util/uacc_hw/rtl/`：collector、candidate engine、divider、top 和 testbench；
- `util/uacc_hw/run_synthesis.py`：统一综合参数、工具版本和结果抽取；
- `util/uacc_hw/report.py`：生成 CSV/JSON、表格和图；
...
先让定点 Python model 与 `util/uacc_gg1_sim.py`/gem5 浮点 Section 14 对齐，再让 RTL 与定点 model 逐 cycle 对齐。测试包括 `M=0`、`S_A=0`、`R1=0`、`R1` 接近 `rho_max*T`、最大 CA²、最大 beta、计数器溢出、负 MU、并列 score 和窗口切换。
...
Section 14 的 HLS 正确性原型已在 `util/uacc_hw/hls/` 拆成三个可独立综合的 top：

| top | 已实现范围 |
|---|---|
| `uacc_section14_collector` | 每 domain 的 `N/M/SA/SA2/n_j/SW`、occupancy maximum、buffer-full/backpressure、跨窗口 arrival history、饱和与 overflow |
| `uacc_section14_cost` | `R1 < rho_max*T` guard、direct queue-cost、beta 修正、128-bit numerator、Q16 reciprocal 和显式输出饱和 |
| `uacc_section14_allocator` | CA² 与 beta shift-EWMA、`R0/R1/R2`、固定成本/容量收益/MU、`{1,2,4}` score、occupancy/backpressure guard 和最高正候选选择 |
